...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for _ in range(self.iterations):
            ...
            for i in self.mdp.getStates():
                if (self.computeActionFromValues(i) != None):
                    values[i] = self.getQValue(i, self.computeActionFromValues(i))
            self.values = values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for s_prime, T in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            V += T * (R + gamma * self.values[s_prime])
        return V
    ...
    def computeActionFromValues(self, state):
        ...
        for i in self.mdp.getPossibleActions(state):
            values[i] = self.getQValue(state, i)
